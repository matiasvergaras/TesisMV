...
for USE_RN50 in USERN50:
    for DS_FLAGS in DSFLAGS:
        for NUM_LABELS in NUMLABELS:
            ...
            output_dir = os.path.join(root_dir, 'outputs', OUT_FOLDER,
              ...
              for meth_name, method in TRANSF_METHODS.items():
                    for base_name, classifier in BASE_CLASSIFIERS.items():
                        ...
                        for j in range(0, K):
                            ...
                            # CC se detendrá si recibe un fold que no contenga algun caso positivo para algun label en pred.
                            if 0 in np.sum(Y_train, axis=0).values:
                                print("All one class")
                                continue
                            else:
                                K_div += 1
                        ...
                        avg_f1 = round(sum_f1/K_div, 4)
                        avg_recall = round(sum_recall/K_div, 4)
                        avg_precision = round(sum_precision/K_div, 4)
                        avg_acc = round(sum_acc/K_div, 4)
                        avg_hl = round(sum_hl/K_div, 4)
                        avg_emr = round(sum_emr/K_div, 4)
                        avg_hs = round(sum_hs/K_div, 4)

                        avg_mr1 = round(sum_mr1/K_div, 4)
                        avg_mr2 = round(sum_mr2/K_div, 4)
                        avg_mr3 = round(sum_mr3/K_div, 4)
                        avg_mr4 = round(sum_mr4/K_div, 4)
                        avg_mr5 = round(sum_mr5/K_div, 4)
